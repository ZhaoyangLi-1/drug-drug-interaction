...
@registry.register_builder("base_dataset")
class BaseBuilder(BaseDatasetBuilder):
    train_dataset_cls = MultimodalDataset
    ...
    def build(self):
        ...
        if build_info.storage is not None:
            datasets["train"] = dataset_cls(
                datapath=build_info.storage,
                use_image="image" in self.config.data_type,
                use_graph="graph" in self.config.data_type,
                is_train=True,
            )
        if build_info.storage_valid is not None:
            datasets["valid"] = dataset_cls(
                datapath=build_info.storage_valid,
                use_image="image" in self.config.data_type,
                use_graph="graph" in self.config.data_type,
            )
        if build_info.storage_test is not None:
